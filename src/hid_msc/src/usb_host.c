...
struct client_s
{
    ...
    struct
    {
        TAILQ_ENTRY(client_s)
        tailq_entry;
        TAILQ_HEAD(tailhead_pending_ep, ep_wrapper_s)
        pending_ep_tailq;
        TAILQ_HEAD(tailhead_idle_ep, ep_wrapper_s)
        idle_ep_tailq;
        TAILQ_HEAD(tailhead_done_ctrl_xfers, urb_s)
        done_ctrl_xfer_tailq;
        union
        {
            struct
            {
    ...
    struct
    {
        TAILQ_HEAD(tailhead_interfaces, interface_s)
        interface_tailq;
    } mux_protected;
...
typedef struct
{
    ...
    struct
    {
        TAILQ_HEAD(tailhead_clients, client_s)
        client_tailq; // List of all clients registered
    } mux_protected;
